...
bool ClientConfig::Save(const wxString& rConfigFileName) 
{
	...
	if (const int * value = name.GetPointer()) { \
		buffer.Printf(_(#name " = " fmt "\n"), *value); \
		file.Write(buffer, wxConvBoxi); \
	}
	
	#define WRITE_INT_PROP(name) \
	if (const int * value = name.GetPointer()) { \
		buffer.Printf(_(#name " = %d\n"), *value); \
		file.Write(buffer, wxConvBoxi); \
	}
	...
	if (PidFile.GetPointer()) {
		...
		buffer.Printf(_("Server\n{\n\tPidFile = %s\n}\n"), pidbuf.c_str());
		file.Write(buffer, wxConvBoxi);
	}
	
	const BoxiLocation::List& rLocations = GetLocations();
	file.Write(_("BackupLocations\n{\n"));
...
bool PropSet(StringProperty& rProp, wxString& msg, const char* desc)
{
	...
	msg.Printf(_("The %s is not set"), desc2.c_str());
	return false;
}
...
bool PropPath(StringProperty& rProp, wxString& msg, bool dir, const char *desc)
{
	...
	if (dir) 
	{
		...
		{
			msg.Printf(_("The %s is not a directory (%s)"),
				desc2.c_str(), path2.c_str());
			return false;
		}
		if (!wxFileName::DirExists(path2))
		{
			msg.Printf(_("The %s does not exist (%s)"),
				desc2.c_str(), path2.c_str());
			return false;
		}
	} 
	else 
	{
		if (wxFileName::DirExists(path2))
		{
			msg.Printf(_("The %s is not a file (%s)"),
				desc2.c_str(), path2.c_str());
			return false;
		}
		if (!wxFileName::FileExists(path2))
...
bool PropWritable(StringProperty& rProp, wxString& msg, const char *desc)
{
	...
	if (wxFileName::FileExists(path2))
	{
		if (wxFile::Access(path2, wxFile::write))
			return true;
		msg.Printf(_("The %s cannot be written (%s)"), 
	...
	else
	{
		...
		if (wxFile::Access(parentPath, wxFile::write))
			return true;
		msg.Printf(_("The %s parent directory "
				"cannot be written (%s)"), 
			desc2.c_str(), parentPath.c_str());
		return false;
	}
...
bool ClientConfig::CheckAccountNumber(wxString* pMsgOut)
{
	...
	{
		*pMsgOut = _("The account number is empty or not valid");
		return false;
	}
...
bool ClientConfig::CheckStoreHostname(wxString* pMsgOut)
{
	...
	{
		*pMsgOut = _("The store hostname is not configured");
		return false;
	}
	...
	{
		*pMsgOut = _("The store hostname is not valid");
		return false;
	}
	...
	unsigned char* pData, wxString* pMsgOut)
{
	...
	{
		*pMsgOut = _("This private key is protected with a passphrase. "
	...
	X509* pCert, wxString* pMsgOut)
{
	...
	commonNameExpected.Printf(_("BACKUP-%x"), accountNumber);

	if (!commonNameExpected.IsSameAs(commonNameActual))
	{
		wxString msg;
		msg.Printf(_("The certificate does not match the "
	...
	EVP_PKEY* pExpectKey, BIO* pActualBio, BIO* pExpectBio, wxString* pMsgOut)
{
	...
	if (expSize != actSize)
	{
		*pMsgOut = _("The certificate file does not match "
			"your private key.");
		return false;
	}
	...
	if (!pExpBuffer)
	{
		*pMsgOut = _("Failed to allocate space for private key data");
		return false;
	}
	...
	wxString* pMsgOut)
{
	...
	{
		*pMsgOut = _("The certificate file does not match your private key.");
		return false;
	}
...
std::auto_ptr<SslConfig> SslConfig::Get(wxString* pMsgOut)
{
	...
	wxString configFileName = _("openssl.cnf");
